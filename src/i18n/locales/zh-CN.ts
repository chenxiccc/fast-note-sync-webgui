export default {
    ...
    "ui.settings.userDatabaseConfig": "数据库增强设置",
    "ui.settings.userDatabaseDesc": "默认 <b>SQLite</b>（免维护，支持并发读/单写），适合个人及小规模部署，能满足个人用户大部分需求。<br/><div class='mt-2 pt-2 border-t border-border/50 opacity-90'><div class='mb-2 text-amber-500 font-medium'>建议改用专业数据库 (PostgreSQL / MySQL) 的场景：</div><ul class='list-disc list-inside space-y-1 mb-1'><li>多用户、多个笔记库且规模极大</li><li>笔记数量或附件体积庞大</li><li>经常遇到并发相关错误</li></ul><div>专业数据库在并发与大规模数据处理上更可靠，但会提高硬件要求与运维成本（需具备运维能力）。</div><div class='mt-2 pt-2 border-t border-border/50 text-xs text-rose-500 font-medium italic'>注意：当修改数据库之后，原数据不会迁移到新数据库，您需要重新强制同步笔记库；另外所有设置必须先通过验证才能更改数据库。</div></div>",
